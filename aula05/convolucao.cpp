...
#include <cstdlib>    // Inclui a biblioteca para uso de atoi (conversão de string para int).
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        if (rank == 0) {
            std::cerr << "Uso: " << argv[0] << " <número de iterações>" << std::endl;
        }
        MPI_Finalize();
        return 1;
    }

    int iterations = std::atoi(argv[1]);  // Converte o argumento da linha de comando para o número de iterações.
